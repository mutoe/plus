...
easemob = {
                    ...
                    var options = {
                        ...
                        success: function (res) {
                            ...
                            _.forEach(res.data, function(value, key) {
                                value.member === undefined ? group_uids.unshift(parseInt(value.owner)) : group_uids.push(parseInt(value.member));
                            });
        ...
        if (TS.BOOT['im:helper']) {
            window.TS.dataBase.transaction('rw?', window.TS.dataBase.room, () => {
                _.forEach(TS.BOOT['im:helper'], function(value, key) {
                    window.TS.dataBase.room.where('[mid+uid]').equals([TS.MID, value['uid']]).first(function(item){
                        if (item === undefined) {
                            var room = {
                                title: value['name'],
                                type: 'chat',
            ...
            window.TS.dataBase.room
                ...
                .toArray().then(function(items){
                    _.forEach(items, function(value, key) {
                        var title = value.type == 'groupchat' ? value.title : _this.users[value.uid]['name'];
                        var avatar = value.type == 'groupchat' ? TS.SITE_URL + '/assets/pc/images/logo.png' : getAvatar(_this.users[value.uid], 50);
                        var to = value.type == 'groupchat' ? value.group : value.uid;

                        var sidehtml = '<dd class="ms_chat" id="ms_chat_' + value.id + '" data-cid="' + value.id + '" data-name="' + title + '"><a href="javascript:;" onclick="easemob.openChatDialog(0, '+ value.id +', '+ to +')"><img src="' + avatar + '"/></a></dd>';
    ...
    setNewCon: function(room){
        var _this = this;
        var title = room.type == 'groupchat' ? room.title : _this.users[room.uid]['name'];
        var avatar = room.type == 'groupchat' ? TS.SITE_URL + '/assets/pc/images/logo.png' : getAvatar(_this.users[room.uid], 50);
        var to = room.type == 'groupchat' ? room.group : room.uid;

        var sidehtml = '<dd class="ms_chat" id="ms_chat_' + room.id + '" data-cid="' + room.id + '" data-name="' + title + '"><a href="javascript:;" onclick="easemob.openChatDialog(0, '+ room.id +', '+ to +')"><img src="' + avatar + '"/></a></dd>';
        ...
        if ($('.chat_dialog').length > 0) {
            ...
            var html = '<li class="room_item" data-type="0" data-cid="' + room['id'] + '" data-uid="' + to + '" data-name="' + room['title'] + '" id="chat_' + room['id'] + '">'
        ...
        window.TS.dataBase.transaction('rw?', window.TS.dataBase.room, () => {
            window.TS.dataBase.room.where('[mid+uid]').equals([TS.MID, uid]).first(function(item){
                ...
                if (item === undefined) {
                    ...
                    window.TS.dataBase.room.add(room).then(function(i){
                        var sidehtml = '<dd class="ms_chat" id="ms_chat_' + i + '" data-cid="' + i + '" data-name="' + easemob.users[uid]['name'] + '"><a href="javascript:;" onclick="easemob.openChatDialog(0, '+ i +', '+ uid +')"><img src="' + getAvatar(easemob.users[uid], 50) + '"/></a></dd>';
                        $('#ms_pinneds').after(sidehtml);
                        easemob.cid = i;
    ...
    setMes: function(txt, user_id){
        ...
        if (user_id == window.TS.MID) {
            html = '<div class="chatC_right">'
                 +      '<img src="' + getAvatar(this.users[user_id]) + '" class="chat_avatar fr">'
    ...
    openChatDialog: function(type, cid, uid) {
        ...
        if (type == 0) {
            ...
            ly.load('/message?type=' + type + '&cid=' + cid + '&uid=' + uid, '', '810px', '572px');
        } else {
